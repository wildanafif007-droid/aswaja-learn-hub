...
function Index() {
  return (
    ...
    <div className="relative min-h-screen bg-gradient-to-br from-emerald-50 to-white">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:py-12 relative z-10">
        <section className="relative overflow-hidden rounded-3xl border border-border shadow-soft">
          ...
          <div className="absolute inset-0 flex flex-col justify-center gap-4 p-6 sm:p-10">
            ...
            <p className="max-w-lg text-sm text-primary-foreground/85 sm:text-base">
              Belajar tradisi Ahlussunnah wal Jama'ah An-Nahdliyah dengan ringkasan LKS TAQWA dan
              kuis interaktif bersertifikat.
            </p>
        ...
        <div className="mt-8 flex flex-col items-center gap-4 rounded-2xl bg-white/80 p-5 shadow-sm backdrop-blur-md md:flex-row md:text-left">
          <img
            src="/tokoh.webp"
            alt="Tokoh Nahdlatul Ulama"
            className="w-48 rounded-xl object-cover shadow-md"
          />
          <p className="italic text-emerald-900 font-medium">
            "Al-Muhafazhotu ‘ala qodimis sholih wal akhdzu bil jadidil ashlah. Memelihara tradisi
            lama yang baik dan mengambil tradisi baru yang lebih baik."
          </p>
        </div>

        <section className="relative z-10 -mt-12 pt-20">
          <h2 className="text-center font-display text-2xl font-bold">Tiga Pilar Ajaran Aswaja</h2>
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {pilar.map((p) => (
              <article
                key={p.title}
                className="group rounded-2xl border border-border bg-white/80 p-6 shadow-sm backdrop-blur-md transition-all hover:-translate-y-1 hover:shadow-lg"
              >
